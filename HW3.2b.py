...
class ChanceNode(Node):
    ...
    def get_expected_utility(self):
        ...
        """
        exp_utility = self.utility  # expected utility initialized with the cost of visiting the current node
        i = 0
        for node in self.futureNodes:
...
class DecisionNode(Node):
    ...
    def get_expected_costs(self):
        ...
        outcomes1 = dict() # dictionary to store the expected cost of future nodes along with their names as keys
        for node in self.futureNode:
            outcomes1[node.name] = node.get_expected_cost()
...
# create C2
C2 = ChanceNode('C2', 15, 0, [T1, T2], [0.7, 0.3])
# create C1
C1 = ChanceNode('C1', 0, 0, [C2, T3], [0.2, 0.8])
# create C3
C3 = ChanceNode('C3', 2, 0, [T4, T5], [0.1, 0.9])

# create D1
D1 = DecisionNode('D1', 0, 0, [C1, C3])

# print the expected cost of D1
print(D1.get_expected_costs())

# print the expected utility of D1
